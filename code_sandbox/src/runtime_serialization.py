"""Serialization helpers for Python runtime outputs.

Used by the code runner bootstrap to convert values returned by server.py into
JSON-safe values before printing the final execution result.
"""
...
def evaluate(expr, locals_for_eval=None):
    if locals_for_eval is None:
        locals_for_eval = {}
...
def to_json(v):
    ...
    a {'_type':..., '_value':...} pair that can be json serialized:
        ...
        non-complex ndarray (assumes each element can be json serialized) -
        > '_type': 'ndarray'
