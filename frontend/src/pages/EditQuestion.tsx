...
import QuestionMetaDataPreview from "../features/QuestionBuilder/sections/QuestionMetadataPreview";
...
export default function EditQuestionPage() {
    const [activePanes, setActivePanes] = useState<EditorOptions[]>(["LivePreview"]);
    ...
    return (
        <div className="space-y-3">
            <div className="rounded-lg border border-border bg-surface p-3">
                ...
                <div className="mb-3">
                    <ServerModeSwitch value={serverMode} onChange={handleServerModeChange} />
                </div>
            ...
            {loading && <div className="text-sm text-text-muted">Loading files...</div>}
                    ...
                    <React.Fragment key={`${v}-${index}`}>
                        ...
                        >
                            {paneRenderMap[v as EditorOptions]?.(ctx) ?? (
                                <div className="p-4 text-muted-foreground">Unknown pane: {v}</div>
                            )}
                        </Panel>
