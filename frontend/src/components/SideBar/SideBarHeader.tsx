...
export default function SideBarHeader({ title, Icon, isOpen, toggle }: HeaderProps) {
    const sidebarState = isOpen ? "open" : "closed";
    return <div className="flex flex-row items-center justify-start gap-2">
        {isOpen && <span className="min-w-0 text-xl truncate pl-4  font-semibold tracking-wide text-text">{title}</span>}
        {Icon && (
