...
export default function AllQuestionTable({
  ...
  const questionSummaryColumns: TableColumn[] = [
    ...
    {
      key: "isAdaptive",
      render: (q) => ((q as QuestionAllRow).isAdaptive ? "Yes" : "No"),
    },
    { key: "status", render: (q) => (q as QuestionAllRow).status },
    {
      key: "created_by",
      render: (q) => (q as QuestionAllRow).created_by || "—",
    },
    {
      key: "institution",
      render: (q) => (q as QuestionAllRow).institution || "—",
    },
  ];
