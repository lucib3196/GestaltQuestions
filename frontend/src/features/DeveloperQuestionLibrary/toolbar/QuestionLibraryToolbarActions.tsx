...
import {
  ...
  type QuestionLibraryToolbarActionId,
  type QuestionLibraryToolbarPopupActionId,
} from "./constants";
type Props = {
  popUp: QuestionLibraryToolbarPopupActionId | null;
  onOpenPopUp: (id: QuestionLibraryToolbarPopupActionId | null) => void;
};

export function QuestionLibraryToolbarActions({ popUp, onOpenPopUp }: Props) {
  ...
  const actionHandlers: Record<QuestionLibraryToolbarActionId, () => void> = {
    copy: async () => {
  ...
  return (
    <div className="flex w-full flex-col gap-3 pt-3 md:flex-row  md:justify-between items-baseline">
      <ToolBarActions
        actions={QUESTION_LIBRARY_TOOLBAR_ACTIONS}
