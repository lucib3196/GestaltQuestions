...
export function parseQuestionPayload(msg: ToolMessage): QuestionPreviewPayload {
  ...
  const filePayload: File[] = Array.isArray(rawFiles)
    ...
    : rawFiles && typeof rawFiles === "object"
      ? Object.entries(rawFiles as Record<string, unknown>).map(
          ([filename, content]) =>
            new File(
...
export const submitFinalQuestionPayload: ToolExecute<
...
export function QuestionReviewCard({
  ...
  const title =
    typeof draftMetadata.title === "string" ? draftMetadata.title : "";
  const [topicsInput, setTopicsInput] = useState(
    Array.isArray(payload.metadata?.topics)
      ? payload.metadata.topics.join(", ")
      : "",
  );
