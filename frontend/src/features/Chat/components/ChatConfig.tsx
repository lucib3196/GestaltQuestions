...
import { ASSISTANTS, type AssistantId } from "../constants/assistants"
...
function AssistantConfig() {
    ...
    return <div className="space-y-2 ">
        <label htmlFor="assistant-select" className="block text-sm font-medium text-text">
            Assistant
        </label>
        <select
            id="assistant-select"
        ...
        {selectedAssistant ? (
            <p className="text-sm text-text-muted">{selectedAssistant.description}</p>
        ) : null}
    </div>
}
...
export default function ChatConfig() {
    return <div className="flex flex-col w-8/10 items-start justify-center mx-auto gap-3">
        <ModelConfig />
        <AssistantConfig />
