...
const getTemplateContent = (
    ...
    const adaptiveMatch = templateSpec.template.find((t) => t.adaptive === isAdaptive);
...
export function CreateQuestionFromBlank() {
    ...
    }, [mode, questionIsAdaptive, setDefaultFiles, setQdata, qdata?.title, qdata?.ai_generated]);
    ...
    return (
        <div className="flex h-full w-full flex-col gap-2 p-2">
            <div className="flex gap-2">
                ...
                />
                <Button
                    name={showTemplateEditor ? "Hide Template Editor" : "Show Template Editor"}
                    onClick={() => setShowTemplateEditor((prev) => !prev)}
                    className="opacity-80"
                />
            </div>

            <PanelGroup direction="horizontal" className="min-h-0 flex-1 items-stretch">
