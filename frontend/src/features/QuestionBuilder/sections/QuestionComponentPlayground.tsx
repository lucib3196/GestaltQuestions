...
const EXAMPLES_BY_TAG: Record<string, string> = {
    ...
    "pl-solution-panel":
        `<pl-solution-panel title="Solution">\n  <p>Subtract 4 on both sides: $2x=10$</p>\n  <p>Then divide by 2: $x=5$</p>\n</pl-solution-panel>`,
    "pl-number-input":
        `<pl-number-input answers-name="x" digits="2" label="$x$"></pl-number-input>`,
...
function buildFallbackMarkup(component: QuestionComponentDoc): string {
    const requiredAttrs = component.attributes
        ...
        .map((attr) => `${attr.htmlAttribute.split("|")[0]}="${getExampleValue(attr)}"`);
    ...
    return (
        <section className="space-y-4">
            <header className="rounded-lg border border-border bg-surface p-4">
                <h2 className="text-lg font-semibold text-text">Question Component Playground</h2>
                <p className="text-sm text-text-muted">
                    Explore and test QuestionRender tags in isolation without creating a full question.
                </p>

                <div className="mt-3 flex flex-wrap items-center gap-2">
                    <label htmlFor="component-select" className="text-sm text-text-muted">
                        Component
                    </label>
                    <select
                        id="component-select"
                    ...
                    <Button
                        ...
                        color="paneMuted"
                        size="sm"
                        onClick={() => setEditorValue(buildComponentMarkup(selectedComponent))}
                    />
                </div>
            </header>
                ...
                <PanelGroup direction="horizontal">
                    <Panel defaultSize={30} minSize={20} className="min-w-[250px]">
                        <div className="h-full overflow-auto border-r border-border p-4">
                            ...
                            <p className="mt-3 text-sm text-text-muted">{selectedComponent.summary}</p>

                            <div className="mt-4">
                                <h4 className="text-sm font-semibold text-text">Children</h4>
                                <p className="text-sm text-text-muted">{selectedComponent.children}</p>
                            </div>

                            <div className="mt-4">
                                <h4 className="text-sm font-semibold text-text">Attributes</h4>
                                <div className="mt-2 space-y-2">
                                    {selectedComponent.attributes.map((attr) => (
                                        <div key={`${selectedComponent.tag}-${attr.prop}`} className="rounded-md border border-border bg-surface-strong p-2">
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-xs font-mono text-text">{attr.htmlAttribute}</span>
                                                <span
                                                    className={
                                                        attr.required
                                            ...
                                            <p className="mt-1 text-xs text-text-muted">{attr.description}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <Panel defaultSize={35} minSize={25}>
                        <div className="h-full border-r border-border">
                            <CodeEditor value={editorValue} setValue={setEditorValue} language="html" />
                        </div>
                    </Panel>
