...
def generate_server_base(state: ServerState) -> ServerState:
    """
    Generates the Python base code: uses semantic search to pull relevant examples
    as guidance and creates the base server.py file.
    """
    ...
    prompt: ChatPromptTemplate = q_retriever_py.format_template(
    ...
    if state.solution_guide:
        solution_prompt = f"""
            ...
            and methodology as the primary reference for the Python (server.py) implementation.
            Expand steps where necessary for clarity, but ensure the structure and logic of the
            Python output closely follow the guide's approach.
...
workflow.add_edge("generate_server_base_py", "add_predefined_values")
