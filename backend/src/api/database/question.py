...
from typing import Any, Dict, Literal, Sequence, Annotated
        ...
        for key, values in data.model_dump(exclude=set(self.metadata_rel)).items():
            ...
            if key in self.metadata_rel:
                ...
                if isinstance(values, list):
                    rel_conds = [
                        filter_conditional(rel_model, lookup_name, v) for v in values
                    ]
            ...
            if isinstance(values, list):
                conds.append(
                    or_(*[filter_conditional(Question, key, v) for v in values])
                )
            else:
                conds.append(filter_conditional(Question, key, values))
