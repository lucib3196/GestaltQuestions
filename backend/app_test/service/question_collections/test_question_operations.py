...
from backend.question.collections import QuestionAlreadyInCollectionError


@pytest.mark.asyncio
async def test_add_question(
    question_collection_service, dev_owner_with_question
) -> None:
    profile = dev_owner_with_question.profile
...
async def test_get_all_questions_returns_added_questions(
    ...
    questions = await question_collection_service.get_questions_for_collections(collection)
...
async def test_get_collection_detail_read_includes_question_ids(
    ...
    collection = await question_collection_service.create_collection(
        dev_owner_with_question.profile,
    ...
    assert detail.question_ids == [dev_owner_with_question.question.id]
